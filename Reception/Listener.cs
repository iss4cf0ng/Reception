...
public class Listener
{
    ...
    void AcceptCallback(IAsyncResult async_result)
    {
        Socket handler = (Socket)async_result.AsyncState;
        Socket client = handler.EndAccept(async_result);
        Victim v = new Victim(client);
        //Task.Run(() => ReadCallback(v));
        handler.BeginAccept(new AsyncCallback(AcceptCallback), handler); //LOOP
        socket.BeginReceive(v.buffer, 0, MAX_BUFFER_LENGTH, SocketFlags.Partial, new AsyncCallback(ReadCallback), v);
    }
