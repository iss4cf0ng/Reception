...
BASE64 PADDING FIX
'''
def b64_fix(data, altchars=b'+/') -> bytes:
    if type(data) == str:
        ...
        try:
            ...
            if command == 1: # KEY EXCHANGE
                if param == 0:
                    ...
                    rsa_pub = xml_to_rsa_key(rsa_pub, private=False)
                    cipher_AES_key = rsa_encrypt(AES_key, rsa_pub)
                    cipher_IV = rsa_encrypt(IV_value, rsa_pub)
                    ...
                    payload = recv[6:].decode('utf-8')
                    try:
                        payload = base64.b64decode(payload)
                    except:
                        payload = b64_fix(payload)
                ...
                if param == 0:
                    payload = recv[6:6 + _len].decode('utf-8')
                    try:
                        payload = base64.b64decode(payload)
                    except:
                        payload = b64_fix(payload)
